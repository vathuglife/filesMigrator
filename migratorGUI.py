...
configLocation = r"E:\My Coding Shits\Python Shits\OS Experiments\GUIExperiments\kivyMD\fileMigrator\ver1.1\configurations.json"
...
<extensionInput@TextInput>
    id: root.textInputId
    text:root.customExtension
    font_style:'H2'
    font_size: '30sp'
    font_style: 'H2'
    size_hint:None,None
    ...
    ScrollView:
        ...
        MDList:
            GridLayout:
                ...
                MD3Card:
                    funcText: "Customize Extensions"
                    imgsource: "puzzle.png"
                    cardId:'custExtenCard'
...
class infoButton(MDIconButton):
    def __init__(self,text,**kwargs):
        ...
        self.customDialogText = text
    def on_release(self,*args):
        Clock.schedule_once(self.showDialog,0.1)
    def showDialog(self,*args):
        self.dialog = MDDialog(
            text="[color=#FFFFFF][size=28][font=Calibri]"+self.customDialogText+"[/color][/size][/font]",
            title="[color=#FFFFFF][size=32][font=Quicksand]"+"Information"+"[/color][/size][/font]",
            md_bg_color=(86/255,72/255,139/255,1),
            size_hint=(None,None),
            width=760,
            height=80,
            buttons=[
...
class extensionInput(TextInput):
    customExtension = StringProperty()
    pass    

#Class to create DISCRETE EXTENSION FIELDS with DISTINCTIVE ATTRIBUTES (specified at the beginning of object creation)
class extensionEditor(RelativeLayout):
    def __init__(self,fieldName,position,iconName,extension,textInputID,**kwargs):
        super().__init__(**kwargs)
        self.extensionLists = configData["fileTypes"]
        execGroup = RelativeLayout()

        tag = Label(text=fieldName,font_size=35,
                    pos=position,font_name="Quicksand",halign='left')
        execGroup.add_widget(tag)
        
        customIcon = Image(source=iconName,pos=(10,165),
                            size_hint=(None,None),width=60,height=60)
        execGroup.add_widget(customIcon)
        
        self.extendInput = extensionInput(customExtension=str(extension),pos=(85,145),
                                          on_text_validate=self.getIdAndUpdate,
                                          cursor_blink=True)
        execGroup.add_widget(self.extendInput)
        
        self.confirmButton = Button(background_normal="loopSmall.png",background_down="loopDark.png",
                                size_hint=(None,None),width=50,height=50,pos=(680,165))
        execGroup.add_widget(self.confirmButton)    
        self.confirmButton.bind(on_release=self.getIdAndUpdate)

        self.realTextID = textInputID
        self.add_widget(execGroup)
    def getIdAndUpdate(self,*args):
        baseInput = self.extendInput.text
        #remove leading and ending double quotes from the list, helping it to add json file.
        baseInputEdited = ast.literal_eval(baseInput) 
        if self.realTextID == "exe":
            self.extensionLists[0] = baseInputEdited
        elif self.realTextID == "doc":
            self.extensionLists[1] = baseInputEdited
        elif self.realTextID == "audio":
            self.extensionLists[2] = baseInputEdited
        elif self.realTextID == "zips":
            self.extensionLists[3] = baseInputEdited
        elif self.realTextID == "images":
            self.extensionLists[4] = baseInputEdited
        
        self.confirmButton.background_normal = 'updateExten.png'
        Clock.schedule_once(self.revertIcon,1.5)
        with open(configLocation,"w") as configFile:
            json.dump(configData,configFile)
    def revertIcon(self,*args):
        self.confirmButton.background_normal = "loopSmall.png"
...
class allDirScreen(Screen):
    def __init__ (self,**kwargs):
        super (allDirScreen,self).__init__(**kwargs)
        
        self.infBtn = infoButton("Shows all saved source directories.")
        self.add_widget(self.infBtn)
    def on_enter(self,*args):
...
class addDirScreen(Screen):
    def __init__ (self,**kwargs):
        ...
        text = "This function allows you to add new source directories."    
        text = text + "\n\n[b]Steps:"
        text = text + "\n1. Click on the dialog box and enter a valid directory."
        text = text + "\n2. Hit Enter/Click Continue to finish.[/b]"
        self.infBtn = infoButton(text)
        self.add_widget(self.infBtn)
...
class changeDestScreen(Screen):
    def __init__ (self,**kwargs):
        super (changeDestScreen,self).__init__(**kwargs)
        text = "This function helps you change the destination folder's location."    
        text = text + "\n\n[b]Steps:"
        text = text + "\n1. Click on the dialog box and enter a new valid destination."
        text = text + "\n2. Hit Enter/Click Continue to finish.[/b]"
        self.infBtn = infoButton(text)
        self.add_widget(self.infBtn)
    ...
    def on_enter(self,*args):
        ...
        self.inputDest = TextInput(font_size=30,pos=(100,245),
    ...
    def updateDest(self,*args):
        if (os.path.isdir(str(self.inputDest.text)) == 0):
            notif = notification("Invalid input. Please try again!",'x-button2.png',(75,75),(320,68),2)
            self.add_widget(notif)
            return 0
        else:
...
class startMigrateScreen(Screen):
    ...
    counter = 1
    def __init__ (self,**kwargs):
        super (startMigrateScreen,self).__init__(**kwargs)
        text = "Starts migrating all files from source folders to the destination directory."    
        self.infBtn = infoButton(text)
        self.add_widget(self.infBtn)
...
class removeDirScreen(Screen):
    ...
    def __init__ (self,**kwargs):
        super (removeDirScreen,self).__init__(**kwargs)
        text = "This function helps you remove unwanted source directories."    
        text = text + "\n\n[b]Steps:"
        text = text + "\n1. Select 1/many directories you want to delete."
        text = text + "\n2. Hit Remove to finish removing."
        text = text + "\n\nIMPORTANT: As a result of Kivy's checkbox bug, after each delete time,"
        text = text + " please click the select all button, and hit it once again to refresh. "
        self.infBtn = infoButton(text)
        self.add_widget(self.infBtn)
...
class customizeExtensionScreen(Screen):
    iconName = StringProperty()
    def on_enter(self,*args):
        extensionList = configData["fileTypes"]
        self.scrollView = ScrollView(size_hint=(None,None),size=(760,330),
                                pos_hint = {'center_x':0.49,'center_y':0.44},
                                always_overscroll=False,
                                bar_inactive_color = (0,0,0,0),bar_color=(1,1,1,1),
                                bar_width=3,
                                do_scroll_x=False)
        self.layout = GridLayout(spacing=-120,size_hint=(None,None),width=800,height=760,cols=1,rows=5)
        tags = ["Executables","Documents","Audio","Zips","Images"]
        tagIndex = 0
        idList = ["exe","doc","audio","zips","images"]
        tagsPos = [(-218,100),(-220,100),(-265,100),(-275,100),(-250,100)]

        icons = ["coding.png","file.png","audio.png","zip.png","gallery.png"]
       
        for num in range(0,5):
            extension = extensionEditor(tags[num],tagsPos[num],icons[num],extensionList[num],idList[num])
            self.layout.add_widget(extension)
    
        self.scrollView.add_widget(self.layout)
        self.add_widget(self.scrollView)
    def getId(self,*args):
        inputId = self.extendInput.text
        print(inputId)
    def on_leave(self,*args):
        self.remove_widget(self.scrollView)


class FilesMigrator(MDApp):
    sm = ScreenManager()
    def build (self):
        self.icon = "file-sharing.png"
